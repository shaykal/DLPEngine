...
object PropertiesUtils {
  ...
  lazy val rulesCreditCardKeywords: String = load(Consts.RULES_CREDITCARD_KEYWORDS)
  lazy val rulesCreditCardIsActive: Boolean = load(Consts.RULES_CREDITCARD_IS_ACTIVE, "false").toBoolean
  lazy val wordsDistance: Int = load(Consts.WORDS_DISTANCE).toInt


  /**
    * <i>load</i> - This method loads the properties from the application.conf file.
